...
void ext4_create_fs_aux_info()
{
	...
	aux_info.bg_desc_blocks =
		DIV_ROUND_UP(aux_info.groups * sizeof(struct ext2_group_desc_ota),
			info.block_size);
...
void ext4_fill_in_sb(int real_uuid)
{
	...
	sb->s_desc_size = sizeof(struct ext2_group_desc_ota);
...
void ext4_update_free()
{
	...
	for (i = 0; i < aux_info.groups; i++) {
		...
		crc = ext4_crc16(crc, &aux_info.bg_desc[i], offsetof(struct ext2_group_desc_ota, bg_checksum));
